...
int mul_q31(int a, int b) {
    ...
    product = product << 1;
    int32_t newprod = (uint32_t)(product>>32);
...
void myfir(int x[], int c[], int y[], int n, int m) {
    // your code here, use add_q31 and mul_q31
    for (int j = 0; j < n-m+1; j++) {
        int32_t sum=(uint32_t)0;
        for (int i = 0; i < m; i++) {
            int32_t temp=mul_q31(c[i],x[j-i+m-1]);
            
            sum=add_q31(sum, temp);   
        }
        y[j]=sum;
    }
}
...
int main(void) {
    ...
    myfir(sin_table, lowpass, y, 20, 4);
